...
static void ltmqServerGetRequestData(LT_Server * pServer, LT_ClientNode * pClientNode, LTMQ_ParseStruct * parseSt)
{
    int ret;
    int  count;
    char * oricontent;
    ...
    if(parseSt->head.flag & LTMQ_PACKET_FLAG_COMPRESS) 
    {
        ...
        requestObj = json_tokener_parse(oricontent);
        if(is_error(requestObj))
        {
        }

        LT_Server_Append(pServer, pClientNode, parseSt->head.requestID, requestObj); 
    ...
    {
        requestObj = json_tokener_parse(parseSt->contentbuf);
        if(is_error(requestObj))
        {
        }
        LT_Server_Append(pServer, pClientNode, parseSt->head.requestID, requestObj);
...
int  ltmqServerParseFunc(const char * recvBuff, int recvSize, LT_Server * pServer, LT_ClientNode * pClientNode)
{
    ...
    parseSt = (LTMQ_ParseStruct * )LT_Server_GetParseStruct(pClientNode);
    pRecv = (char *)recvBuff;
    parseSize = 0;
    restSize = recvSize; 
    ...
    while(restSize > 0)
    {
        ...
        else if(parseSt->state == LTMQ_PARSE_CONTENT)
        {
            ...
            else
            {
                ...
                //get a requestdata
                ltmqServerGetRequestData(pServer, pClientNode, parseSt);
...
int  ltmqServerSerialFunc(LT_RequestInfo * pInfo, char ** buf, int * buflen)
{
    ...
    replybuf = jo_to_json_string(pInfo->replyData);

    contentbuf = NULL;

    BZERO(&head, sizeof(LTMQ_PacketHead));

    head.requestID = pInfo->requestID;
    head.oricontentlen = strlen(replybuf) + 1;
    ...
    if(head.oricontentlen >= 1024)
    {
        ...
        ret = compress(contentbuf, &contentlen, (unsigned char  *)replybuf, head.oricontentlen);
    ...
    *buf = (char *)malloc(*buflen);
    ...
    {
        BCOPY(replybuf, *buf + LTMQ_PACKET_HEAD_SIZE, head.contentlen);
    }
